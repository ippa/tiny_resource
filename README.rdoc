...
I liked the feeling of control only using 1 abstract controller where I can easily see all the code (and modify it) as startingpoint instead of a "full resource plugin".
It's not configurable as resoruceplugins. If you need to change it use normal class overloading. Or just modify tiny_resource.rb. It's one file to much around with. You can do it :).
